...
class GlobalToolsLibrary:
    _instance = None
    @classmethod
    def get_instance(cls) -> 'GlobalToolsLibrary':
        if cls._instance is None:
            cls._instance = GlobalToolsLibrary()
...
class SimpleLLMContext(LLMProcessContext):
    ...
    def load_action_set_from_config(self,preset,config:Dict[str,str]) -> Dict:
        ...
        if enable_actions:
            for action_id in enable_actions:
                ai_func = GlobalToolsLibrary.get_instance().get_tool_function(action_id)
                if ai_func:
                    result[action_id] = LLMProcessContext.function2action(ai_func)
                else:
                    func_set = GlobalToolsLibrary.get_instance().get_function_set(action_id)
                    if func_set:
                        for _func_id in func_set:
                            ai_func = GlobalToolsLibrary.get_instance().get_tool_function(_func_id)
                            if ai_func:
    ...
    def load_function_set_from_config(self,preset,config:Dict) -> Dict[str,AIFunction]:
        ...
        if enable_functions:
            for func_id in enable_functions:
                ai_func = GlobalToolsLibrary.get_instance().get_tool_function(func_id)
                if ai_func:
                    result[func_id] = ai_func
                else:
                    func_set = GlobalToolsLibrary.get_instance().get_function_set(func_id)
                    if func_set:
    ...
    def load_from_config(self,config:Dict[str,str]) -> bool:
        ...
        if preset:
            self.parent:SimpleLLMContext = GlobalToolsLibrary.get_instance().get_preset_context(preset)
